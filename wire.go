...
import (
	...
	"github.com/evermos/boilerplate-go/internal/domain/products/repository"
...
var domainProducts = wire.NewSet(
	...
	wire.Bind(new(services.Service), new(*services.ServiceImpl)),
	// Repository interface and implementation
	repository.ProvideRepo,
	wire.Bind(new(repository.Repository), new(*repository.RepositoryImpl)),
	// Producer interface and implementation
	// producer.NewSNSProducer,
	// wire.Bind(new(producer.Producer), new(*producer.SNSProducer)),
)
...
var routing = wire.NewSet(
	wire.Struct(new(router.DomainHandlers), "ProductsHandler"),
	handlers.ProvideProductsHandler,
	router.ProvideRouter,
)
